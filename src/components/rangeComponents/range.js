...
export const Range = ({ minValue, maxValue, rangeValues=[] }) => {

  const hasFixedValues = rangeValues && rangeValues.length > 0;

  const INIT_POSITION_MIN = hasFixedValues ? rangeValues[0] : minValue;
  const INIT_POSITION_MAX = hasFixedValues ? rangeValues[rangeValues.length - 1] : maxValue;
  const LIMIT = 200
  const minColor='#282c34';
  const maxColor='#e4a46c';
  ...
  const steps = rangeValues
  const [step, setStep] = useState(0);
  const stepsRight = rangeValues
  ...
  const onMouseMoveLeftFixed =  useCallback( (e) => {
    if (isDraggingMin.current) {
  ...
  const onMouseMoveLeftNormal = useCallback( (e) => {
    if (isDraggingMin.current) {
      setPositionMin((positionMin) =>  positionMin + e.movementX);
    }
  }, []);

  const onMouseMoveLeft = (hasFixedValues) ? onMouseMoveLeftFixed : onMouseMoveLeftNormal;

const onMouseMoveRightFixed =  useCallback( (e) => {
  if (isDraggingMax.current) {
    const valueToCheck =e.movementX*5;
...
const onMouseMoveRightNormal =  useCallback( (e) => {
  if (isDraggingMax.current) {
  ...
  const onMouseMoveRight = (hasFixedValues) ? onMouseMoveRightFixed : onMouseMoveRightNormal;
